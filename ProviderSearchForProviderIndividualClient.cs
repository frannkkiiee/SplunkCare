...
namespace Nehta.VendorLibrary.HI
{
    ...
    {
        internal ProviderSearchForProviderIndividualPortType providerSearchForProviderIndividualClient;
        ...
        /// HI service name.
        /// </summary>
        public const string HIServiceOperation = "ProviderSearchForProviderIndividual";
        ...
        public ProviderSearchForProviderIndividualClient(Uri endpointUri, ProductType product, QualifiedId user, QualifiedId hpio, X509Certificate2 signingCert, X509Certificate2 tlsCert)
        {
        ...
        /// <param name="signingCert">Certificate to sign the soap message with.</param>
        /// <param name="tlsCert">Certificate for establishing TLS connection to the service.</param>
        public ProviderSearchForProviderIndividualClient(string endpointConfigurationName, ProductType product, QualifiedId user, QualifiedId hpio, X509Certificate2 signingCert, X509Certificate2 tlsCert)
        {
        ...
        private void InitializeClient(string endpointUrl, string endpointConfigurationName, X509Certificate2 signingCert, X509Certificate2 tlsCert, ProductType product, QualifiedId user, QualifiedId hpio)
        {
            ...
            ProviderSearchForProviderIndividualPortTypeClient client = null;

            if (!string.IsNullOrEmpty(endpointUrl))
            {
                var address = new EndpointAddress(endpointUrl);

                client = new ProviderSearchForProviderIndividualPortTypeClient(tlsBinding, address);
            }
            else if (!string.IsNullOrEmpty(endpointConfigurationName))
            {
                client = new ProviderSearchForProviderIndividualPortTypeClient(endpointConfigurationName);
            }
            ...
            if (client != null)
            {
                ...
                providerSearchForProviderIndividualClient = client;
            }
        }
        ...
        public void Dispose()
        {
            ClientBase<ProviderSearchForProviderIndividualPortType> lClient;

            if (providerSearchForProviderIndividualClient is ClientBase<ProviderSearchForProviderIndividualPortType>)
            {
                lClient = (ClientBase<ProviderSearchForProviderIndividualPortType>)providerSearchForProviderIndividualClient;
